...
export function Header({ locale, dict }: HeaderProps) {
  ...
  const deepen = useTransform(scrollY, [0, 80], [0, 1]);
  ...
  return (
    <header className="sticky top-0 z-40 h-14 print:hidden">
      ...
      {/* Always-on frosted bar so nav stays readable over hero photo */}
      <div className="absolute inset-0 border-b border-line/80 bg-canvas/82 backdrop-blur-md" />
      <motion.div
      ...
      <Container className="relative flex h-full items-center justify-between gap-4">
        ...
        <Link href={localePath(locale)} className="group shrink-0" onClick={() => setMenuOpen(false)}>
          ...
          >
            <motion.div
              className="flex h-7 w-7 items-center justify-center border border-accent/60 bg-canvas/60 group-hover:border-accent group-hover:bg-accent transition-all duration-200"
            ...
            <span className="hidden text-xs font-medium uppercase tracking-[0.18em] text-ink group-hover:text-accent transition-colors duration-200 sm:block">
              {site.name}
            </span>
          </motion.div>
        </Link>
        ...
        <nav aria-label={dict.nav.primary} className="hidden md:flex items-center gap-1">
          {nav.map((item, i) => (
            ...
            >
              <Link
                href={item.href}
                className="relative px-3 py-1.5 text-[11px] font-medium uppercase tracking-[0.16em] text-ink/80 transition-colors hover:text-accent after:absolute after:bottom-0 after:left-3 after:right-3 after:h-px after:scale-x-0 after:bg-accent after:transition-transform after:duration-300 hover:after:scale-x-100"
              >
                {item.label}
              </Link>
            </motion.div>
