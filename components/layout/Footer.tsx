...
export function Footer({ dict, locale }: { dict: Dictionary; locale: Locale }) {
  ...
  return (
    <footer className="mt-auto border-t border-line bg-surface print:hidden overflow-hidden">
        <Container className="flex flex-col items-center justify-center gap-4 py-6 text-sm sm:flex-row sm:gap-8">
        <motion.a
          href={`mailto:${site.email}`}
          className="max-w-full shrink-0 break-all text-center text-muted transition-colors hover:text-accent"
